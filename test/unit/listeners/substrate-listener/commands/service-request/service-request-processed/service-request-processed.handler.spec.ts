...
describe('Service Request Processed Handler Event', () => {
  ...
  it('should called ServiceRequesProcessedHandler from command bus', async () => {
    const queryServiceInvoiceByIdSpy = jest.spyOn(serviceRequestQuery, 'queryServiceInvoiceById').mockImplementation();
    const setOrderPaidSpy = jest.spyOn(orderCommand, 'setOrderPaid').mockImplementation();
    ...
    when(queryServiceInvoiceByIdSpy)
      .calledWith(substrateServiceMock.api, requestData[1].toHuman().requestHash)
      .mockReturnValue(SERVICE_INVOICE_RETURN);

    const serviceRequestProcessedCommand: ServiceRequestProcessedCommand = new ServiceRequestProcessedCommand(requestData, mockBlockNumber());
    ...
    expect(queryServiceInvoiceByIdSpy).toHaveBeenCalledWith(substrateServiceMock.api, requestData[1].toHuman().requestHash)
    expect(setOrderPaidSpy).toHaveBeenCalled();
    expect(setOrderPaidSpy).toHaveBeenCalledWith(substrateServiceMock.api, substrateServiceMock.pair, SERVICE_INVOICE_RETURN.orderId);
  });
});