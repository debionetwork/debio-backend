...
describe('Health Controller Unit TEsts', () => {
  ...
  let defaultConnectionMock: Connection;
   ...
   const healthCheckServiceMockFactory: () => MockType<HealthCheckService> = jest.fn(() => ({
     check: jest.fn(),
   }));

   const typeOrmHealthIndicatorMockFactory: () => MockType<TypeOrmHealthIndicator> = jest.fn(() => ({
     pingCheck: jest.fn(),
   }));

   const memoryHealthIndicatorMockFactory: () => MockType<MemoryHealthIndicator> = jest.fn(() => ({
     checkHeap: jest.fn(),
   }));

   const diskHealthIndicatorMockFactory: () => MockType<DiskHealthIndicator> = jest.fn(() => ({
     checkStorage: jest.fn(),
   }));

   const elasticsearchHealthIndicatorMockFactory: () => MockType<ElasticsearchHealthIndicator> = jest.fn(() => ({
     isHealthy: jest.fn(),
   }));

   const substrateHealthIndicatorMockFactory: () => MockType<SubstrateHealthIndicator> = jest.fn(() => ({
     isHealthy: jest.fn(),
   }));
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: TypeOrmHealthIndicator, useFactory: typeOrmHealthIndicatorMockFactory },
        { provide: MemoryHealthIndicator, useFactory: memoryHealthIndicatorMockFactory },
        { provide: DiskHealthIndicator, useFactory: diskHealthIndicatorMockFactory },
        { provide: ElasticsearchHealthIndicator, useFactory: elasticsearchHealthIndicatorMockFactory },
        { provide: SubstrateHealthIndicator, useFactory: substrateHealthIndicatorMockFactory },
  ...
  it('should health check', () => {
    ...
    expect(typeOrmHealthIndicatorMock.pingCheck).toHaveBeenCalledWith('database', { connection: defaultConnectionMock });
    expect(typeOrmHealthIndicatorMock.pingCheck).toHaveBeenCalledWith('location-database', { connection: dbLocationConnectionMock });
    expect(memoryHealthIndicatorMock.checkHeap).toHaveBeenCalledTimes(1);
    expect(memoryHealthIndicatorMock.checkHeap).toHaveBeenCalledWith('memory heap', 300 * 1024 * 1024);
    expect(diskHealthIndicatorMock.checkStorage).toHaveBeenCalledTimes(1);
    expect(diskHealthIndicatorMock.checkStorage).toHaveBeenCalledWith('disk health', {
