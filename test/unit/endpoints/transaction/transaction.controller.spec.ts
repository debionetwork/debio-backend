...
describe('Transaction Controller Unit Test', () => {
  ...
  it('should update transaction', async () => {
    ...
    when(transactionLoggingService.getLoggingByOrderId).calledWith(PARAM.order_id).mockReturnValue(EXPECTED_VALUE);

    expect(transactionController.submitTransactionHash(PARAM)).resolves.toEqual( { status: 'ok' } );
    expect(transactionServiceSpy).toHaveBeenCalled();
    expect(transactionLoggingService.getLoggingByOrderId).toHaveBeenCalled();
    expect(transactionLoggingService.getLoggingByOrderId).toHaveBeenCalledWith(PARAM.order_id);
    
    await Promise.resolve();
    expect(transactionLoggingService.updateHash).toHaveBeenCalled();
    expect(transactionLoggingService.updateHash).toHaveBeenCalledWith(EXPECTED_VALUE, PARAM.transaction_hash);
    await Promise.resolve();
