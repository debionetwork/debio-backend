...
import { SubstrateController, RegistrationRole, GetDbioOnRegisterDto, WalletBindingDTO } from "./substrate.controller";
  ...
  describe("Controller Testing", () => {
    ...
    describe("When getDbioPreRegister is called if user send request to get-dbio-pre-register", () => {
      ...
      it("getDbioPreRegister called when account id is already registered, status response is 208 and response text is User has already registered.", async () => {
        ...
        responseDbioPreRegister = await substrateController.getDbioPreRegister(req, res);
        expect(responseDbioPreRegister.status).toHaveBeenCalledWith(208);
        expect(responseDbioPreRegister.send).toHaveBeenCalledWith("User has already registered");
      });
      ...
      it("getDbioPreRegister called when role is not found", async () => {
        ...
        expect(responseDbioPreRegister.send).toHaveBeenCalledWith('role not found');
      });
      ...
      it("getDbioPreRegister called when account id is not found", async () => {
        ...
        responseDbioPreRegister = await substrateController.getDbioPreRegister(req, res);
        expect(responseDbioPreRegister.status).toHaveBeenCalledWith(400);
        expect(responseDbioPreRegister.send).toHaveBeenCalledWith('accountId is required');
      });
    });
    ...
    describe("When walletBinding is called if user send request to wallet-binding", () => {
      ...
      it("walletBinding called, when debioApiKey is not right, some substrateService method mustn't called", async () => {
        ...
        responseWalletBinding = await substrateController.walletBinding(req, res, debioApiKey);
        expect(substrateService.getSubstrateAddressByEthAddress).not.toHaveBeenCalled();
        expect(substrateService.bindEthAddressToSubstrateAddress).not.toHaveBeenCalled();
      });
      ...
      it("walletBinding called, response status must 200 when header is right", async () => {
        ...
        expect(responseWalletBinding.send).toHaveBeenCalledWith(`eth-address ${ethAddress} bound to ${substrateAddress}`);
      });
      ...
      it("when walletBinding is running, getSubstrateAddressByEthAddress must called, bindEthAddressToSubstrateAddress must called", async () => {
        ...
        responseWalletBinding = await substrateController.walletBinding(req, res, debioApiKey);
        expect(substrateService.getSubstrateAddressByEthAddress).toHaveBeenCalled();
        expect(substrateService.getSubstrateAddressByEthAddress).toHaveBeenCalledWith(ethAddress);
        expect(substrateService.bindEthAddressToSubstrateAddress).toHaveBeenCalled();
        expect(substrateService.bindEthAddressToSubstrateAddress).toHaveBeenCalledWith(ethAddress, accountId);
      });
      ...
      it("walletBinding called, response status must 400 when header is not right", async () => {
        ...
        expect(responseWalletBinding.send).toHaveBeenCalledWith('debio-api-key header is required');
      });
    });
  });
});