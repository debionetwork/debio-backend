...
export class RewardFundsHandler
  implements ICommandHandler<RewardFundsCommand>
{
  ...
  async execute(command: RewardFundsCommand) {
    ...
    try {
      const lastReward = await this.rewardLoggingService.getLastRewardByAccountId(reward['accountId'])
