...
class EtheretumServiceMock {
    ...
    convertCurrency(fromCurrency = 'ETH', toCurrency = 'DAI', amount: number | string,) {
        return {};
    }
}
...
describe.only("EthereumService", () => {
    ...
    it("should call getGasEstimationFee method with expected params", async() => {
        const getGasEstimationFeeSpy = jest.spyOn(ethereumService, 'getGasEstimationFee');
        const from: string = "";
        const to: string = "";
        const data = {};
    ...
    it("should call convertCurrency method with expected params", async() => {
        ...
        const fromCurrency: string = 'ETH';
        const toCurrency: string = 'DAI';
        const amount: number | string = 20;
        ethereumService.convertCurrency(fromCurrency, toCurrency, amount);
        expect(convertCurrencySpy).toHaveBeenCalledWith(fromCurrency, toCurrency, amount);
    });
});